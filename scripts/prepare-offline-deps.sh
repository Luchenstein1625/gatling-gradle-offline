...
docker run --rm --entrypoint sh gatling-dependency-cache:local -lc "tar -C /opt -czf - gradle" > "$OUT_DIR/gradle-8.14.3.tar.gz"
...
# - gradle-8.14.3: 3 archivos (primeros dos de 50 MB, tercero con el resto)
split -d -a 3 -b 50M "$OUT_DIR/gradle-8.14.3.tar.gz" "$OUT_DIR/gradle-8.14.3.tar.gz.part-"
...
rm -f "$OUT_DIR/gradle-8.14.3.tar.gz" "$OUT_DIR/gradle-cache.tar.gz"

echo "Done. Generated:"
ls -lh "$OUT_DIR"/gradle-8.14.3.tar.gz.part-* "$OUT_DIR"/gradle-cache.tar.gz.part-*
